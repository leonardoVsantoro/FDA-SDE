...
def run(n, r, dt, mu, sigma, x0, N, sigma_noise, h_m, h_G, it=0):
    ...
    intSIGMAsquared = np.array([ simps((SIGMA**2)[:t_ix+1], time_grid[:t_ix+1]) for t_ix in range(N)])
       
    
    # mu -- drift     
    MUHAT = dMHAT/MHAT
    rmse_drift = ((((MU-MUHAT)**2)[2:-2]).mean())**.5


    # sigma -- diffusion 
    SIGMASQUAREDHAT_tria = np.array([get_sigma2_TRIANGULAR(
        MHAT, dMHAT, GHAT, D1GHAT, MUHAT)(t) for t in time_grid])
    rmse_tria = ((((SIGMA **2-SIGMASQUAREDHAT_tria)**2)[2:-2]).mean())**.5


    if sigma_noise == 0:
        SIGMASQUAREDHAT_diag_D =  np.array([get_sigma2_DIAGONAL_fromDiag(
            MHAT, dMHAT, diag_GHAT, diag_dGHAT, MUHAT)(t) for t in time_grid])
        rmse_diag_D = ((((SIGMA**2-SIGMASQUAREDHAT_diag_D)**2)[2:-2]).mean())**.5
    else:
        rmse_diag_D = np.nan
    ...
    rmse_diag_T = ((((SIGMA**2-SIGMASQUAREDHAT_diag_T)**2)[2:-2]).mean())**.5





    # int sigma -- integrated diffusion 
   
    intSIGMAsquaredHAT_T = int_vol_T(GHAT, dMHAT, MHAT,N)
    RMSE_intSIGMAsquared_T = ((intSIGMAsquaredHAT_T - intSIGMAsquared)**2).mean()**.5

        
    if sigma_noise == 0:
         intSIGMAsquaredHAT_D = int_vol_D(diag_GHAT, dMHAT, MHAT,N)
         RMSE_intSIGMAsquared_D = ((intSIGMAsquaredHAT_D - intSIGMAsquared)**2).mean()**.5
    
    else:
        intSIGMAsquaredHAT_D = int_vol_D(GDIAGHAT, dMHAT, MHAT,N)
        RMSE_intSIGMAsquared_D = ((intSIGMAsquaredHAT_D - intSIGMAsquared)**2).mean()**.5
        

    
    
    return rmse_drift, rmse_tria, rmse_diag_D, rmse_diag_T, MUHAT, SIGMASQUAREDHAT_tria,SIGMASQUAREDHAT_diag_D, SIGMASQUAREDHAT_diag_T,  intSIGMAsquaredHAT_D , RMSE_intSIGMAsquared_D, intSIGMAsquaredHAT_T , RMSE_intSIGMAsquared_T
                        



def int_vol_D(D, dM, M,N):
    time_grid = np.linspace(0,1,N)
    return np.array([ D[t_ix] - D[0] - 2* simps(dM[:t_ix+1]*D[:t_ix+1]/M[:t_ix+1], time_grid[:t_ix+1])
        for t_ix in range(N)
    ])
...
def int_vol_T(G, dM, M,N):
    time_grid = np.linspace(0,1,N)
    _ = np.array( [ 
        np.mean([ G[t_ix, s_ix] - G[0,0] \
                - 2 * simps( dM[:t_ix+1]*np.diag(G)[:t_ix+1]/M[:t_ix+1], time_grid[:t_ix+1]) \
                - simps(dM[t_ix:s_ix+1]*G[t_ix, t_ix:s_ix+1]/M[t_ix:s_ix+1], time_grid[t_ix:s_ix+1])    
        for s_ix in np.arange(N)[t_ix:]])   
              for t_ix in range(N)])    
    return _ - _[0]
